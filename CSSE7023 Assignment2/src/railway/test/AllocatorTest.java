...
public class AllocatorTest {
    ...
    public void basicTest01() throws Exception {
        ...
        List<List<Segment>> expectedAllocation = asList(
                expectedAllocationArray);

        // the actual result of the allocation
        List<List<Segment>> actualAllocation = Allocator.allocate(occupied,
                requested);
     ...
     */
    @Test
    public void basicTest08() throws Exception {
        // arrays describing the occupied, requested and expected routes
        Segment[][] occupiedArray = {
                { new Segment(sections[0], endPoints[0], 6, 10)},
                { new Segment(sections[2], endPoints[4], 5, 7),
                        new Segment(sections[3], endPoints[7], 0, 1) },
                {new Segment(sections[3], endPoints[6], 5, 9)}};

        Segment[][] requestedArray = {
                { new Segment(sections[1], endPoints[2], 3, 4)},
                { new Segment(sections[1], endPoints[2], 8, 12),
                new Segment(sections[3], endPoints[6], 0, 6)},
                {new Segment(sections[1], endPoints[2], 5, 12)}};

        Segment[][] expectedAllocationArray = {
                { new Segment(sections[1], endPoints[2], 3, 4)},
                { new Segment(sections[1], endPoints[2], 8, 12),
                    new Segment(sections[3], endPoints[6], 0, 4)},
                {new Segment(sections[1], endPoints[2], 5, 7)}};
